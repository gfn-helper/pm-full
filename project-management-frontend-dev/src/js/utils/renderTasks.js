...
export async function createTaskList(task, type, projectType, sprint, userMap = null) {
  let ifSprint = `hidden`;
  if (type === 'backlog') {
    ifSprint = ``;
  }
...
async function getProjectColumns() {
  ...
  if (columnsCache && cachedProjectId === pid) return columnsCache;
  const project = (await projectService.getProjectById(pid)).result;
  columnsCache = project.columns;
  cachedProjectId = pid;
  return columnsCache;
}
