...
export async function renderBacklogView(
  ...
  const idToTask = new Map(allTaskObjects.map((t) => [t._id, t]));
  const incompleteBacklogTaskObjects = backlogTasks
  ...
  currentSprints.forEach(async (sprint) => {
    ...
    const sprintTaskObjects = sprint.tasks
      .map((id) => idToTask.get(id))
      .filter(Boolean);
    dropdownEvent(sprint);

    if (!sprintTaskObjects.length) {
      document
      ...
      if (project.result.projectType === 'kanban') {
        await renderSprintTasks(sprint, sprintTaskObjects, 'kanban');
      } else {
        await renderSprintTasks(sprint, sprintTaskObjects, '');
      }
    }
