...
export async function renderBacklogTasks(
  ...
  const userIds = [
    ...new Set(backlogTasks.flatMap((t) => [t.assignee, t.reporter]).filter(Boolean)),
  ];
  let userMap = {};
  if (userIds.length) {
    const usersResp = await taskService.getMultipleUsers(userIds);
  ...
  for (const task of backlogTasks) {
    if (task.status !== project.columns[project.columns.length - 1]) {
      promiseArray.push(
        createTaskList(task, 'backlog', project.projectType, null, userMap)
      );
    }
  }
